...
X, Y, U_h, X_U_rb_star, lb, ub = prep_data(hp["n_x"], hp["n_y"], hp["n_s"])
plt.contour(X, Y, U_h[:, :, 0])
plt.show()
V = get_pod_bases(U_h, hp["n_x"], hp["n_y"], hp["n_s"], hp["eps"])
...
# Splitting data
n_t_train = int(hp["train_val_ratio"] * hp["n_s"])

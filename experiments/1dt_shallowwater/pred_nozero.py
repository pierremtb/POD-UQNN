...
for row, mu_lhs in enumerate([mu_lhs_in, mu_lhs_out]):
    ...
    x = np.linspace(hp["x_min"], hp["x_max"], hp["n_x"])
    idx = np.random.choice(X_v_samples.shape[1], n_samples, replace=False)
    for col, idx_i in enumerate(idx):
        ...
        if row == 0:
            ...
            ax.axhline(X_i[time, 0] + 0.02, color="w", ls="-.")
        ...
        if False:
            ax.legend()
plt.tight_layout()
savefig(os.path.join("results", "podbnn-1dswt-graph-meansamples-h"), False)
...
for row, mu_lhs in enumerate([mu_lhs_in, mu_lhs_out]):
    ...
    for col, idx_i in enumerate(idx):
        ...
        if False:
            ax.legend()
plt.tight_layout()
savefig(os.path.join("results", "podbnn-1dswt-graph-meansamples-u"), False)
